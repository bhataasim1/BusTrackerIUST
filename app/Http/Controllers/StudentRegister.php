...
class StudentRegister extends Controller
{
    public function Student_Register()
    {
        ...
        return view('studentregister', compact('busRoutes'));
    }

    public function RegisterSuccess(Request $request) {
        $request->validate([
            'reg__no' => ['required', 'unique:students,reg__no', 'regex:/^IUST\d{10}$/', 'min:14','max:14', new RegNoUniqueAcrossDB],
        ...
        if ($data) {
            ...
            return redirect('student/login')->with('status', 'User already registered');
        } else {
            if ($password == $confirm_password) {
                ...
                $student->save();
                return redirect('student/login')->with('status', 'Registration Successful');
            } else {
                return redirect('studentregister')->with('status', 'Password and Confirm Password do not match');
            }
        }  
    }
}
